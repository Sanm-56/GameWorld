...
create table if not exists public.progreso_nivel (
  ...
  nivel integer not null default 1 check (nivel between 1 and 365),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
alter table public.progreso_nivel
add constraint progreso_nivel_nivel_check check (nivel between 1 and 365);

create table if not exists public.recompensas_nivel (
  id bigint generated by default as identity primary key,
  nivel integer not null check (nivel between 1 and 365),
...
alter table public.recompensas_nivel
add constraint recompensas_nivel_nivel_check check (nivel between 1 and 365);
...
alter table public.recompensas_desbloqueadas
add constraint recompensas_desbloqueadas_nivel_check check (nivel between 1 and 365);

create table if not exists public.historial_temporadas_nivel (
  id bigint generated by default as identity primary key,
  usuario_id text not null,
  temporada_id text not null references public.temporadas(id),
  nivel_anterior integer not null check (nivel_anterior between 1 and 365),
  nivel_nuevo integer not null check (nivel_nuevo between 1 and 365),
  xp_anterior integer not null default 0 check (xp_anterior >= 0),
  xp_nuevo integer not null default 0 check (xp_nuevo >= 0),
  fecha timestamptz not null default now(),
...
as $$
declare
  nivel_limpio integer := least(greatest(coalesce(nivel_objetivo, 1), 1), 365);
  nivel_actual integer := 1;
  acumulado integer := 0;
begin
...
as $$
  select (array[
    ...
    'Inmortal',
    'Leyenda Maxima'
  ])[least(15, greatest(1, ceil(least(greatest(coalesce(nivel_actual, 1), 1), 365) / 25.0)::integer))];
$$;
...
from generate_series(1, 365) as nivel
